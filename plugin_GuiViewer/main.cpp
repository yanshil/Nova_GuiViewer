...
class Simulation_Factory : public RenderableFactory
{
  ...
  public:
    ...
    virtual std::vector<std::unique_ptr<Renderable>> Create_Multiple(ApplicationFactory &app, std::string path)
    {
        ...
        fs::path load_path(directory_name + "/0/");
        ...
        BOOST_FOREACH (fs::path const &p, std::make_pair(it, eod))
            if (fs::is_regular_file(p))
            {
                ...
                if (filename.find_last_of('.') != std::string::npos)
                {
                    ...
                    if (extension == "obj")
                    {
                        renderables.push_back(std::move(std::unique_ptr<Renderable>(new Object_Renderable<float, 3>(app, directory_name, number_of_frames))));
