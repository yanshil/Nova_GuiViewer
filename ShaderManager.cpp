...
ShaderManager::GetShader(const std::string& name)
{
    ...
    if( res == _shaderRepo.end() ){
        ...
        return std::unique_ptr<Shader>( new Shader(program_id) );           
    }
    else{
        return std::unique_ptr<Shader>( new Shader(res->second) );           
    }
}

std::string
ShaderManager::ReadShader( std::string type, std::string path ){
    std::string code = "";
    if( path != "" ){
...
unsigned int 
ShaderManager::LoadFromFiles( std::string name )
{
...
unsigned int 
ShaderManager::LoadFromString( const char* vertex_shader,
