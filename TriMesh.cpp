...
void TriMesh::GenMesh(Cuboid cuboid_, std::vector<Hole> hs_)
{
    FILE *stream;
    vertex_list.Clear();
    triangle_list.Clear();
    ...
    if (hs_.size() == 0) 
    {
        ...
        for(int i = 0; i <= 2; i++)
        {
            triangle_list.Append(Vector<int,3>({i,i+4,i+1}));
            triangle_list.Append(Vector<int,3>({i+4,i+4+1,i+1}));
        }
        
        triangle_list.Append(Vector<int,3>({3,7,0}));
        triangle_list.Append(Vector<int,3>({7,4,0}));
        /* code */
    }
    ...
    else
    {
        ...
        for(int i = 0; i < mid.numberoftriangles; i++)
        {
            triangle_list.Append(Vector<int,3>({mid.trianglelist[3*i]+mid.numberofpoints,
                                                    mid.trianglelist[3*i+1]+mid.numberofpoints,
                                                    mid.trianglelist[3*i+2]+mid.numberofpoints}));
        }
        ...
        for(int i = 0; i < hs_.size(); i++)
        {
            ...
            triangle_list.Append(Vector<int,3>({4+hs_[i].segments*i+j,4+hs_[i].segments*i+j+mid.numberofpoints,4+hs_[i].segments*i+j+1}));    
            triangle_list.Append(Vector<int,3>({4+hs_[i].segments*i+j+mid.numberofpoints,4+hs_[i].segments*i+j+mid.numberofpoints+1,4+hs_[i].segments*i+j+1}));
            
            }
            
            triangle_list.Append(Vector<int,3>({4+hs_[i].segments*i+hs_[i].segments-1,4+hs_[i].segments*i+hs_[i].segments-1+mid.numberofpoints,4+hs_[i].segments*i}));
            triangle_list.Append(Vector<int,3>({4+hs_[i].segments*i+hs_[i].segments-1+mid.numberofpoints,4+hs_[i].segments*i+mid.numberofpoints,4+hs_[i].segments*i}));
        }
    ...
    triangulated_surface=Triangulated_Surface<double>(vertex_list,triangle_list);


    char filename[32] = "output.obj";
    stream = fopen(filename, "w+");
        for(int i=0;i<vertex_list.size();i++)
    fprintf(stream,"v %f %f %f\n",vertex_list[i][0],vertex_list[i][1],vertex_list[i][2]);
